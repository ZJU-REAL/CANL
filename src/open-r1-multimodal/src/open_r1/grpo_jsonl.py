...
def get_vlm_module(model_name_or_path):
    if "qwen" in model_name_or_path.lower() or "ui-tars" in model_name_or_path.lower():
...
def main(script_args, training_args, model_args):
    ...
    question_prompt = vlm_module_cls.get_question_template(task_type=script_args.task_type, model_size=training_args.model_size, model_type=training_args.model_type)
...
# </tool_call>""".replace("{{screen_width}}", str(example['width'])).replace("{{screen_height}}", str(example['height']))
#                             }
#                         ]
#                     },
                    {

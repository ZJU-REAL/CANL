...
class GRPOConfig(TrainingArguments):
    ...
    omniparser_bbox_path: str = field(
        default="xxx",
        metadata={
            "help": "jsonl file path parsed by Omniparser before, containing all bbox information."
        }
    )
    scale: bool = field(
        default=False,
        metadata={
            "help": "Whether to scale image by image processor."
        }
    )
    points2point_method: str = field(
        default="confidence",
        metadata={
            "help": "Method for voting multi points to single point, ex. rkde, confidence, distance_voting"
        }
    )
    is_binary_reward: bool = field(
        default=True,
        metadata={
            "help": "Whether using binary reward."
                    "If set True, 1 reward for point inside bbox, 0 reward for point outside bbox."
                    "If set False, applying point to point gaussian distance as reward"
        }
    )
    threshold: float = field(
        default=0.05,
    )
    downsample_fac: int = field(
        default=1,
        metadata={
            "help": "voting number = rollout * downsample_fac"
        }
    )
    negative_learning: bool = field(
        default=False
    )
    positive_learning: bool = field(
        default=False
    )
    ...
    save_optimizer_and_scheduler: bool = field(
        default=False
    )